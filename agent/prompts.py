...
def get_log_interval_by_depth(
    well_name: str, min_depth: float, max_depth: float, depth_col: str
) -> QIWell:
    """
...
def plot_logs_from_well(log_list: list[str], depth_col: str):
    """
...
def plot_crossplot_insitu_condition(x_attr: str, y_attr: str, facies_list: list[str]):
    """
...
def plot_crossplot_fluid_sub_condition(
    x_attr: str, y_attr: str, facies_list: list[str]
):
    """
...
def perform_fluid_substitution(
    hc_type: str, hc_sat: float, vsh_cut: float, mod_Sw: bool
) -> QIWell:
    """
...
def compute_facies_separability(log_name: str, facies_list: list[str]):
    """
...
def plot_depth_trend_insitu_condition(log_name: str, depth_col: str):
    """
...
def plot_depth_trend_fluid_sub_condition(log_name: str, depth_col: str):
    """
...
def generate_upscale_well(window: float, stride: float):
    """
...
plot_crossplot_insitu_condition_schema = FunctionSchema(
    plot_crossplot_insitu_condition
).to_ollama()
plot_crossplot_fluid_sub_condition_schema = FunctionSchema(
    plot_crossplot_fluid_sub_condition
).to_ollama()
perform_fluid_substitution_schema = FunctionSchema(
    perform_fluid_substitution
).to_ollama()
compute_facies_separability_schema = FunctionSchema(
    compute_facies_separability
).to_ollama()

plot_depth_trend_insitu_condition_schema = FunctionSchema(
    plot_depth_trend_insitu_condition
).to_ollama()
plot_depth_trend_fluid_sub_condition_schema = FunctionSchema(
    plot_depth_trend_fluid_sub_condition
).to_ollama()
...
plot_crossplot_insitu_condition_def = json.dumps(
    plot_crossplot_insitu_condition_schema, indent=2
)
plot_depth_trend_insitu_condition_def = json.dumps(
    plot_depth_trend_insitu_condition_schema, indent=2
)
perform_fluid_substitution_def = json.dumps(perform_fluid_substitution_schema, indent=2)
# FluidSubstitute Agent
plot_crossplot_fluid_sub_condition_def = json.dumps(
    plot_crossplot_fluid_sub_condition_schema, indent=2
)
plot_depth_trend_fluid_sub_condition_def = json.dumps(
    plot_depth_trend_fluid_sub_condition_schema, indent=2
)
compute_facies_separability_def = json.dumps(
    compute_facies_separability_schema, indent=2
)
...
def generate_retriever_sys_prompt():
    retriever_prompt_template = """<|start_header_id|>system<|end_header_id|>
    ...
    example = """
            ...
            """
    return retriever_prompt_template + retriever_tools_def + example


def generate_insitu_sys_prompt():

    insitu_prompt_template = """<|start_header_id|>system<|end_header_id|>
    ...
    example = """
        ...
        return object is  <tool_call>{"name": "perform_fluid_substitution", "arguments": {"hc_type": "oil", "hc_sat": 0.9}}</tool_call>
        """

    return insitu_prompt_template + insitu_tools_def + example


def generate_fluid_sub_sys_prompt():

    fluid_sub_prompt_template = """<|start_header_id|>system<|end_header_id|>
    ...
    fluid_sub_tools_def = f"""
        ...
        """
    example = """
        For example, user's query: can you go get the well BA-18 from 1763 to 2311 m MD. 
        Do fluid substitution for gas with 0.9 saturation. After that, plot its AI depth trend log
        return object is <tool_call>{"name": "plot_depth_trend_fluid_sub_condition", "arguments": {"log_name": "AI", "depth_col": "MD"}}</tool_call>
        """

    return fluid_sub_prompt_template + fluid_sub_tools_def + example
